...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
    ...
    var windowWidth = document.querySelector("#randomPizzas").offsetWidth; // Optimize?
  ...
  function changePizzaSizes(size) {
	var pizzactn = document.getElementsByClassName('randomPizzaContainer'); 
	...
	var dx = determineDx(pizzactn[0], size);
	var newwidth = (pizzactn[0].offsetWidth + dx) + 'px';
	
	for (var i = 0; i < pizzactn.length; i++) {
      //var dx = determineDx(pizzactn[i], size);
      //var newwidth = (pizzactn[i].offsetWidth + dx) + 'px';
	  //console.log(newwidth);
      pizzactn[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  //var phase = Math.sin((document.body.scrollTop / 1250) + (itemslen % 5));
  
  
  var phase;
  var currentypos = latestypos / 1250;
  ...
  for (var i = 0; i < items.length; i++) {
	...
	phase = Math.sin(currentypos + (i % 5));	
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 40; i++) {
	...
	//elem.setAttribute("style", "backface-visiblity:hidden;");
	//elem.style.backface-visiblity = "hidden";
